...
public:
	...
	void updatePicInfo(SxPicBufInfo &picInfo);
...
private:
	...
	bool m_bUseMaxQueueSize;					//!< Use max.buffer queue size for defined acq. buffer size
	CxImageMetadata	aMetadata;					//!? Image metadata after acq. start
	bool m_bIsSimulator;						//!? True for camera simuator instances.
	bool m_ForceContextUpdate;					//!? Update context for each image
};
