...
#if _DEBUG
// Disables automatic parameters update on timer what interrupts debugging (e.g. temperature reading is triggered each second)
//#define DEBUG_DISABLE_PARAMETERS_TIMER_UPDATE
#endif
...
public:
    ...
    bool readIfInvalidated(XICORE_HANDLE hCamera, CxValue &vValue, const QString &sInvalidator, int *piRetVal = NULL) const;
...
public:
    ...
    QString m_sUnit;        //!< Unit of this parameter
...
public:
    ...
    int m_iReadApiError;    //!< Last API error when reading the value of this parameter. 
...
public:
    ...
    bool readFromCamera(XICORE_HANDLE hCamera);      //!< Reads current value from the camera. Error code is stored to m_iReadApiError.
...
public:
    ...
    QString m_sDarkFilename;        //!< File with dark frame for shading correction
    QString m_sMidFilename;         //!< File with light frame for shading correction
	bool	m_bIsActive;			//!< Is shading preset being currently used
...
public:
    ...
    static bool isShadingActive(CxCameraParameters *pPars);      //!< Returns true if shading is switched on the camera (TODO: remove after parameter exists in xiApi)
...
public:
	...
	ExColorFilterArray getCFAPattern(XICORE_HANDLE hCamera);
	...
	QVector<int> collectSymmetricBinDecOptions(const QString& xiPrmHorizontal, const QString& xiPrmVertical);
	bool checkBinDecParams(const QString& xiPrmHorizontal, const QString& xiPrmVertical);

public:
    bool addParam(const CxCameraParam& aParam);     //!< Adds new parameter to our list
	bool removeParam(QString xiApiName);     //!< Removes existing parameter from our list
