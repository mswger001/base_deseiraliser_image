...
#define DTOFREG_VAR	"DoTOF"						//Flag to do automatically trigger one dummy frame.
...
MM40_API MM40_RETURN __cdecl mmGetDeviceExt ( 
			IN  DWORD nIndex,					// Index
...
/**
   \brief Returns temperature of the image sensor
   
   Allows the user to get current value of camera's chip

   @param[in] hDevice			handle of the specified device
   @param[in,out] pTchip			temperature of the chip
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmGetImageSensorTemperature(HANDLE hDevice, float * pTchip );

/**
   \brief Returns not calibrated temperature of the image sensor
   ...
   @return MM40_OK on success, error value otherwise.
 */
MM40_API MM40_RETURN __cdecl mmGetImageSensorTemperatureRaw(HANDLE hDevice, float * pTchip );
